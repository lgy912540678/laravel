...
<div class="search_location">
    <a href="http://www.lenovo.com.cn/">首页</a>&gt;<a href="">笔记本</a>&gt;<span>"笔记本"</span>
</div>
<!-- location end -->
...
<!--                <span class="searchKeyRed"style="display: none;"><em class="globalSearchKey"></em><i class="del"></i></span>-->
        ...
        <div class="tab_body">
            <!-- 过滤条件部分 -->
            <form action="/ppclist" method="get">
        ...
        <div class="search_pro">
            <div class="search_pro_con">
                ...
                <li style="font-size:11px;"><img src="/admin/upload/{{ explode(',',($v->Goods_pic))[0] }}" width="200" height="200" alt=""><div>商品名:{{ $v->Goods_name }}</div><div>商品简介:{{ $v->Goods_body }}</div><div style="color:red;">商品价格:{{ $v->Goods_price }}</div><div><a href="/details/{{ $v->Goods_id }}">查看详情</a> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<a href="javascript:doStore({{ $v->Goods_id }})">收藏</a></div></li>
                @endforeach
        ...
        <ul>
        	<li style="display:inline;"><a href="http://copylenovo.com/ppclist?@if(!empty($where['Goods_name']))Goods_name={{ $where['Goods_name'] }} &@endif @if(!empty($where['Goods_price']))Goods_price[]={{ $where['Goods_price'][0] }}&Goods_price[]={{ $where['Goods_price'][1] }} &@endif page=1"><<</a></li>
        	@for($i = 0;$i < $count;$i++)
        	<li style="display:inline;"><a href="http://copylenovo.com/ppclist?@if(!empty($where['Goods_name']))Goods_name={{ $where['Goods_name'] }} &@endif @if(!empty($where['Goods_price']))Goods_price[]={{ $where['Goods_price'][0] }}&Goods_price[]={{ $where['Goods_price'][1] }}&@endif page={{ $i+1 }}">{{ $i+1 }}</a></li>
        	@endfor
        	
        	<li style="display:inline;"><a href="http://copylenovo.com/ppclist?@if(!empty($where['Goods_name']))Goods_name={{ $where['Goods_name'] }} &@endif @if(!empty($where['Goods_price']))Goods_price[]={{ $where['Goods_price'][0] }}&Goods_price[]={{ $where['Goods_price'][1] }}&@endif page={{ $count }}">>></a></li>
        </ul>
        <!-- {{ $ppclist->appends($where)->links() }} -->
        </div>
    </div>
</div>
